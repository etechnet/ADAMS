...
To compile the C++ source code:
	...
	o Edit the "Makefile.unix.inc.<product>.<OS>" file
	  appropriate for the CORBA product and operating system you
	  are using. If there is not already such a file then create it
	  based on a "Makefile.unix.inc.<product>.<OS>" file for another
	  product or operating system.

	o Run the following command:
	      ln -s Makefile.unix.inc.<product>.<OS> Makefile.unix.inc

	o Go into the "cxx" directory and type "make -f Makefile.unix".
	  This will compile the source code and produce a file called
	  "libcorbautil.a" in the top-level directory.
...
following:
	...
	o If you have Orbix on your computer then edit
	  "Makefile.unix.inc.orbix.<OS>" and follow the instructions
	  in the comments.

	o If you have Orbacus on your computer then edit
	  "java/Makefile.unix" and set th "OB_JAR" variable to point
	  to the "OB.jar" file from your Orbacus installation.

	o Go into the "java" directory.

	o Type "make -f Makefile.unix <target>" where target is one of
	  the following:
