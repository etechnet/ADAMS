...
*VERSION = \'3.00';                        # the version of jamal
...
# we are currently processing, who included who, and at what line in which file) 
@FILE = ();
%LINE = ();
...
#  {#define X/a/b=ab}
#  {X/baba/kutya}
# then the expected result is 'babakutya'. This means that the piece of text
...
=pod
=section jsexinstall
=H Install a j-SEX module interactive

This function is onvoked when the user uses the option 'T<-i>'. There are no
arguments and no return value from this function. The function communicates with
the user using the standard input and the standard output and copies the j-SEX
extension files it finds in the actual directory into one of the directories
that are listed in the T<@INC> global Perl variable letting the user choose
one destination interactively.
...
sub jsexinstall {
  ...
  if( $#ARGV == -1 ){
    print <<END;
I am going to install the jamal j-SEX modules
that I find in the actual directory. These are:
END

    opendir(D,".") or die "Can not open diretory .\n";
    @file = grep /\.pm$/ , readdir(D);
    closedir D;
    if( $#file == -1 ){
      print "There are no j-SEX extension files in the current working directory.\n";
      return;
      }
    }else{
    print <<END;
I am going to install the specified jamal j-SEX modules.
These are:
END
    while( $#ARGV > -1 ){
      push @file , pop @ARGV;
...
REPEATLABEL:
  print <<END;
Your Perl installation has the following
...
END
  ...
  if( $n < 1 or $n > $i ){
    print <<END;
The number you have typed is invalid. I need an integer
number between 1 and $i. Press enter to try again
...
END
  ...
  my $dir = $inc[$n];
  print "Creating directory $dir/jamal\n";
  mkdir "$dir/jamal",0777;
  ...
  for (@file){
    ...
    $fc = <F>;
    close F;
    # if the j-SEX file was specified it may contain the path
    # remove them
    s/^.*\\//;
    s/^.*\///;
    print "Writing $dir/jamal/$_\n";
    open(F,">$dir/jamal/$_") or die "Can not open $dir/jamal/$_";
    print F $fc;
    close F;
    print "File copied\n";
    }
  print "All files were installed. Ensure that the files have the proper permission.\n";
  }
