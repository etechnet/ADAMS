...
static void close_directory_list(pExecuteObject pEo,
  ...
  if( pDCO == NULL )return;
 	...
 	if( pDCO->dp[i] == NULL)return;
 	/* APK 21-Nov-2003 ---- */

  for( j=0 ; j<pDCO->dp[i]->cFileNames ; j++ )FREE(pDCO->dp[i]->ppszFileName[j]);
...
When you call T<SLEEP(n)> the operating system is called telling it that the code does not need the CPU for T<n> seconds. During this time the program is suspended and the operating system executes other programs as needed. The code is guaranteed to return from the function T<SLEEP> not sooner than T<n> seconds, but usually it does return before the second T<n+1> starts.
*/
COMMAND(SLEEP)
#if NOTIMP_SLEEP
NOTIMPLEMENTED;
...
NOTIMPLEMENTED;
  ...
  iError = HOOK_CREATEPROCESS(pszCommandLine, &lPid);
...
/* APK - Nov 2002 - Pass on the not started error */
  if( iError == FILESYSE_NOTSTARTED ) {
    ERROR(COMMAND_ERROR_NOTSTARTED);
    }
  else {
    ERROR(iError);
    }
...
If the program does not finish during the specified period then the function alters the third argument, which has to be a variable and raises error. In this case the argument T<pid_v> will hold the PID of the external program. This value can be used in the error handling code to terminate the external program.
