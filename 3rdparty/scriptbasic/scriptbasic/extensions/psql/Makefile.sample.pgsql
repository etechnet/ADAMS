CC=gcc -W -Wall -O2
# the result is shared and all sub libraries should be linked static
LDOPTIONS=-shared -Bstatic
LD=ld
CCFLAGS=
...
all: headers scriba cftc pgsql.so cgi.so hash.so re.so libscriba.a sbhttpd ux.so mt.so basic.conf dbg.so
...
curl.so : curlintrfc.o
	$(LD) $(LDOPTIONS) -o $@ curlintrfc.o -lcurl -lcurl -lssl -lcrypto -ldl -lc
...
curlintrfc.o : extensions/curl/curlintrfc.c
	$(CC) $(CFLAGS) -c -o curlintrfc.o extensions/curl/curlintrfc.c

s_curlintrfc.o : extensions/curl/curlintrfc.c
	$(CC) $(CFLAGSS) -c -o curlintrfc.o extensions/curl/curlintrfc.c
...
gd.so : extensions/gd/gdinterf.c
	$(CC) -shared -static -o $@ extensions/gd/gdinterf.c -lc -lpng -lz -lgd

zlib.so : extensions/zlib/zlib.c
	$(CC) -shared -static -o $@ extensions/zlib/zlib.c -lc -lz
...
install.sh : install.sh.jam
	perl jamal.pl install.sh.jam install.sh
