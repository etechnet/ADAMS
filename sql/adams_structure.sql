...
--
-- Host: localhost
-- Generato il: Gen 18, 2013 alle 14:34
...
-- Database: `adams_asp`
--
CREATE DATABASE `adams_asp` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `adams_asp`;

-- --------------------------------------------------------

--
-- Definition of table `tab_inputnode`
--

DROP TABLE IF EXISTS `tab_inputnode`;
CREATE TABLE `tab_inputnode` (
  `CODN7_NAZ` int(5) DEFAULT NULL,
  `CODN7_INT` int(5) DEFAULT NULL,
  ...
  `NODE_CMT` varchar(12) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `t_iors` (
  ...
  `ior` text NOT NULL,
  KEY `node_id` (`node_id`,`process_name`),
...
CREATE TABLE IF NOT EXISTS `t_process` (
  ...
  `schedule_flag` int(1) NOT NULL DEFAULT '0',
  ...
  `log_output` enum('Table','File','Pipe') NOT NULL DEFAULT 'Table',
  `log_pipe_cmd` varchar(512) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `t_process_group_members` (
  ...
  KEY `group_id` (`group_id`,`process_id`),
...
CREATE TABLE IF NOT EXISTS `t_process_groups` (
  ...
  `bind_node` varchar(128) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;
...
--
-- Limiti per la tabella `t_iors`
--
ALTER TABLE `t_iors`
  ADD CONSTRAINT `t_iors_ibfk_2` FOREIGN KEY (`process_name`) REFERENCES `t_process` (`process_name`) ON DELETE SET NULL ON UPDATE SET NULL,
  ADD CONSTRAINT `t_iors_ibfk_1` FOREIGN KEY (`node_id`) REFERENCES `t_node` (`id`) ON DELETE SET NULL ON UPDATE SET NULL;

--
-- Limiti per la tabella `t_node`
--
ALTER TABLE `t_node`
  ADD CONSTRAINT `t_node_ibfk_2` FOREIGN KEY (`process_group_id`) REFERENCES `t_process_groups` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE;

--
-- Limiti per la tabella `t_process_group_members`
--
ALTER TABLE `t_process_group_members`
  ADD CONSTRAINT `t_process_group_members_ibfk_1` FOREIGN KEY (`group_id`) REFERENCES `t_process_groups` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
-- Database: `adams_log`
--
CREATE DATABASE `adams_log` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `adams_log`;

-- --------------------------------------------------------

--
-- Struttura della tabella `t_log_config`
--
