...
#include <mdm_server_factory_bus.h>
#include "mdm_server_factory_busadaptor.h"
...
#define SERVER_LAUNCH_TIMEOUT_SECS		15

using namespace std;

static mdm_server_factory_server_impl * servant = 0;
...
{
	int exit_status = 0;
	AdamsServer adams_server ( argc, argv, "mdm_server_factory" );

	AppLogStream = adams_server.logger();

	// DBus init
	mdm_server_factory_bus * server_factory_bus = new mdm_server_factory_bus();
	new Mdm_server_factory_busAdaptor ( server_factory_bus );

	QDBusConnection connection = QDBusConnection::sessionBus();
	connection.registerObject ( "/mdm_server_factory_bus", server_factory_bus );
	connection.registerService ( "net.etech.adams.mdm_server_factory_bus" );

	// Instance main garbage class as a separate thread
	mdm_server_factory server_factory;

	try {

		// Create the servant.
		servant = new mdm_server_factory_server_impl ( adams_server.poa_hierarchy(), "mdm_server_factory_server", true , SERVER_LAUNCH_TIMEOUT_SECS );
		servant->_remove_ref();
