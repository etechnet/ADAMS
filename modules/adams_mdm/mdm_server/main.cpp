...
static mdm_server_server_impl * servant = 0;
...
{
	int exit_status = 0;
	AdamsServer adams_server ( argc, argv, "mdm_server" );
	...
	try {
		...
		servant = new mdm_server_server_impl ( adams_server.poa_hierarchy(), "mdm_server_server", true , 0 );
		mdm_server_server_var servervar = servant->_this();
		servant->_remove_ref();

		// narrow the server factory and push ior back to it
		CorbaClient<mdm_server_factory_server> my_factory;
		if ( !my_factory.connect ( "mdm_server_factory", adams_server.getNode() ) ) {
