...
#include <ssm_garbage_bus.h>
#include "ssm_garbage_busadaptor.h"
#include "ssm_garbage_server_impl.h"
...
static ssm_garbage_server_impl * servant = 0;
...
{
	int exit_status = 0;
	AdamsServer adams_server ( argc, argv, "ssm_garbage" );

	AppLogStream = adams_server.logger();

	// DBus init
	ssm_garbage_bus * garbage_bus = new ssm_garbage_bus();
	new Ssm_garbage_busAdaptor ( garbage_bus );

	QDBusConnection connection = QDBusConnection::sessionBus();
	connection.registerObject ( "/ssm_garbage_bus", garbage_bus );
	connection.registerService ( "net.etech.adams.ssm_garbage_bus" );
	...
	try {
		...
		// Create the servant.
		servant = new ssm_garbage_server_impl ( adams_server.poa_hierarchy(), "ssm_garbage_server", true , 0 );
		servant->_remove_ref();
